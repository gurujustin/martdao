...
function BondPurchase({ bond, slippage, recipientAddress }) {
  ...
  const setMax = () => {
    ...
    if (bond.maxBondPrice * bond.bondPrice < Number(bond.balance)) {
      ...
      maxQ = bond.balance;
      // if (bond.name == "gla_usdc_lp" || bond.isFour) {
              ...
              <div className="help-text">
                <em>
                  <Typography variant="body1" align="center" color="textSecondary">
                    First time bonding <b>{displayName}</b>? <br /> Please approve Gla Dao to use your{" "}
                    <b>{displayName}</b> for bonding.
                  </Typography>
                </em>
              </div>
            ) : (
              <FormControl className="gla-input" variant="outlined" color="primary" fullWidth>
