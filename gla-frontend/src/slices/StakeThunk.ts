...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const glaContract = new ethers.Contract(addresses[networkID].GLA_ADDRESS as string, ierc20Abi, signer);
    const sglaContract = new ethers.Contract(addresses[networkID].SGLA_ADDRESS as string, ierc20Abi, signer);
    let approveTx;
    let stakeAllowance = await glaContract.allowance(address, addresses[networkID].STAKING_HELPER_ADDRESS);
    let unstakeAllowance = await sglaContract.allowance(address, addresses[networkID].STAKING_ADDRESS);
    ...
    try {
      if (token === "gla") {
        ...
        approveTx = await glaContract.approve(
          addresses[networkID].STAKING_HELPER_ADDRESS,
          ethers.utils.parseUnits("1000000000", "gwei").toString(),
        );
      } else if (token === "sgla") {
        approveTx = await sglaContract.approve(
          addresses[networkID].STAKING_ADDRESS,
          ethers.utils.parseUnits("1000000000", "gwei").toString(),
        );
      }

      const text = "Approve " + (token === "gla" ? "Staking" : "Unstaking");
      const pendingTxnType = token === "gla" ? "approve_staking" : "approve_unstaking";
...
export const changeStake = createAsyncThunk(
  ...
  async ({ action, value, provider, address, networkID, callback }: IActionValueAsyncThunk, { dispatch }) => {
    ...
    staking = new ethers.Contract(addresses[networkID].STAKING_ADDRESS as string, GlaStaking, signer);
